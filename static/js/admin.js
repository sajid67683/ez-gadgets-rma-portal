...
function printFromAdmin(r) {
    try {
        ...
        if (!window.jspdf) {
            alert("Error: jsPDF library not loaded. Please add the script tag to your HTML.");
            return;
        }
        ...
        const drawSection = (yOffset, label) => {
            ...
            doc.text(label.toUpperCase(), rightMargin, yOffset + 20, { align: "right" });
            ...
            doc.setFont("helvetica", "normal");
            doc.setTextColor(0, 0, 0);
            const cleanIssue = (r.issue || "No description provided").replace(/[^\x00-\x7F]/g, "");
